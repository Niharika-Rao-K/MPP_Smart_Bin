...
app = FastAPI(title="Smart Bin RAG & Web3 Engine")
...
PRIVATE_KEY = os.getenv("PRIVATE_KEY", "YOUR_PRIVATE_KEY_HERE")  # Wallet that owns/deployed contract
CONTRACT_ADDRESS = os.getenv("CONTRACT_ADDRESS", "0x8685fAc372696060B479bC70a3F58368fdF7dc0a")
...
def mint_reward_tokens(recipient_wallet: str, amount: int = 10):
    """Mints ERC-20 tokens on Sepolia testnet to the user wallet."""
    if not w3.is_address(recipient_wallet):
        raise ValueError(f"Invalid Ethereum wallet address: {recipient_wallet}")
...
async def evaluate_sensor_fusion(
    ...
    try:
        ...
        # Trigger real Web3 token minting if a valid wallet is provided
